...
const Textarea = props => {
    ...
    const Append = () => {
        if (state === 'saving') {
            return (
                <div className={`flex absolute top-0 ${actionLocation}-0 items-center pt-1.5 p${actionLocation.charAt(0)}-3`}>
    ...
    return (
        <div className={wrapperClassName}>
            ...
            <div className="relative">
                <textarea id={id} name={name} className={inputClasses} rows={rows} {...attributes} />
                <Append />
            </div>
            {
                (helpText !== '' || error !== '')
